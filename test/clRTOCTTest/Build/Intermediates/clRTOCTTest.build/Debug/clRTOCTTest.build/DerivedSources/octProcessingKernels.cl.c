...
#include "octProcessingKernels.cl.h"

static void initBlocks(void);

// Initialize static data structures
static block_kernel_pair pair_map[3] = {
      { NULL, NULL },
      { NULL, NULL },
      { NULL, NULL }
};

static block_kernel_map bmap = { 0, 3, initBlocks, pair_map };
...
void (^octPostProcessingKernel_kernel)(const cl_ndrange *ndrange, cl_float* FourierTransformCmplx, cl_float* referenceAScan, cl_uint outputAScanLength, cl_uint numBScans, cl_uint numAScansPerBScan, cl_uint ascanAveragingFactor, cl_uint bscanAveragingFactor, cl_uint imageFormatStride, cl_float minVal, cl_float maxVal, cl_float* envBScan, cl_float* logEnvBScan, cl_float* Sum, cl_float* SAM, cl_float* AttenuationDepth, cl_char* bscanBmp) =
^(const cl_ndrange *ndrange, cl_float* FourierTransformCmplx, cl_float* referenceAScan, cl_uint outputAScanLength, cl_uint numBScans, cl_uint numAScansPerBScan, cl_uint ascanAveragingFactor, cl_uint bscanAveragingFactor, cl_uint imageFormatStride, cl_float minVal, cl_float maxVal, cl_float* envBScan, cl_float* logEnvBScan, cl_float* Sum, cl_float* SAM, cl_float* AttenuationDepth, cl_char* bscanBmp) {
  int err = 0;
  ...
  err |= gclSetKernelArgAPPLE(k, 7, sizeof(imageFormatStride), &imageFormatStride, &kargs);
  err |= gclSetKernelArgAPPLE(k, 8, sizeof(minVal), &minVal, &kargs);
  err |= gclSetKernelArgAPPLE(k, 9, sizeof(maxVal), &maxVal, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 10, envBScan, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 11, logEnvBScan, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 12, Sum, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 13, SAM, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 14, AttenuationDepth, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 15, bscanBmp, &kargs);
...
static void initBlocks(void) {
  ...
  dispatch_once(&once,
    ^{ int err = gclBuildProgramBinaryAPPLE("OpenCL/octProcessingKernels.cl", "", &bmap, build_opts);
       if (!err) {
          assert(bmap.map[0].block_ptr == octPreProcessingKernel_kernel && "mismatch block");
          bmap.map[0].kernel = clCreateKernel(bmap.program, "octPreProcessingKernel", &err);
...
static void RegisterMap(void) {
  ...
  bmap.map[0].block_ptr = octPreProcessingKernel_kernel;
