...
//	char* sourceFile = "/Users/phtomlins/Google Drive/OCT Control Software - octView/clOCTKernels/20140520/octKernels.cl\0";
	...
	err = clBuildProgram(_clOCTProgram, NUM_DEVICES_TO_BUILD_FOR, &_deviceIDs[_selectedDeviceIndex], NULL, NULL, NULL); 
	if (err != CL_SUCCESS)
	{
		//
    ...
    if (err != CL_SUCCESS)
        return err;
	_imageKernel = clCreateKernel(_clOCTProgram, "ConvertFloatArrayToGreyScaleImage", &err);
	if (err != CL_SUCCESS)
		return err;
		...
		)
{
	...
	//
	deviceSpectra = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(short) * _totalInputSpectraLength, NULL, &err);
	if (err != CL_SUCCESS)
		return err;
	//
	...
	if (err != CL_SUCCESS)
		return err;
	deviceBScanBmp = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(unsigned char) * (_totalAScansPerBScan * 3 + _outputImageHeight * _imageFormatStride), NULL, &err);	// attenuation depth along each a-scan
	if (err != CL_SUCCESS)
		return err;
...
__kernel void octPostProcessingKernel(
						0 __global const float* FourierTransformCmplx,				// 1. Input, the Fourier transform of the pre-processed spectra	
						1 __global const float* referenceAScan,				// 2. Reference a-scan subtracted from envelope of FT
						2 const cl_uint outputAScanLength,				// 3. Length of the 1D Fourier transform			
						3 const cl_uint numBScans,						// 4.
						4 const cl_uint numAScansPerBScan,				
						5 const cl_uint ascanAveragingFactor,
						6 const cl_uint bscanAveragingFactor,
						7 __global float* envBScan,							
						8 __global float* logEnvBScan,						
						9 __global float* Sum,
						10 __global float* SAM,		// Currently unused
						11 __global float* AttenuationDepth	// Currently unused
						)	*/
	cl_int err = 0;
	...
	if (err != CL_SUCCESS) return err;
	err = clSetKernelArg(_postProcessingKernel, 7, sizeof(cl_mem), &deviceEnvBScan);	// Envelope of BScan
	if (err != CL_SUCCESS) return err;
	err = clSetKernelArg(_postProcessingKernel, 8, sizeof(cl_mem), &deviceLogEnvBScan);	// Log envelope - usually used for B-Scan images
	if (err != CL_SUCCESS) return err;
	err = clSetKernelArg(_postProcessingKernel, 9, sizeof(cl_mem), &deviceSum);	// Log envelope - usually used for B-Scan images
	if (err != CL_SUCCESS) return err;
	err = clSetKernelArg(_postProcessingKernel, 10, sizeof(cl_mem), &deviceSAM);	// Log envelope - usually used for B-Scan images
	if (err != CL_SUCCESS) return err;
	err = clSetKernelArg(_postProcessingKernel, 11, sizeof(cl_mem), &deviceAttenuationDepth);	// Log envelope - usually used for B-Scan images
	if (err != CL_SUCCESS) return err;
