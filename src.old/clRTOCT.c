...
DLL SaveBitmap(char* path, float* sourceArray, unsigned int width, unsigned int sourceHeight, unsigned int outputHeight, float minVal, float maxVal)
{
    //
   ...
   // unsigned short paddedWidth = (unsigned short)ceil(((double)width * (double)bytesPerPixel)/4.0);
    unsigned int paddingWidth; // = paddedWidth - width;
    ...
    paddingWidth = (unsigned int)(requiredWidthInBytes - actualWidthInBytes);
    //
    ...
    for (h=0;h<outputHeight; h++)
    {
        for (w=0;w<width; w++)
        {
            ...
            bytePixel = (unsigned int)bmpPixel;
